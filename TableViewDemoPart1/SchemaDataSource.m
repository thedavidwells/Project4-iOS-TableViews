...
static NSString *moviesURLString = @"http://cwolf.cs.sonoma.edu/~dwells/movies/dbInterface.py?rType=movies";
static NSString *theaterURLString = @"http://cwolf.cs.sonoma.edu/~dwells/movies/dbInterface.py?rType=movieTheaters";
static NSString *showtimeURLString = @"http://cwolf.cs.sonoma.edu/~dwells/movies/dbInterface.py?rType=showtimes";
static NSString *moviesAtTheaters = @"http://cwolf.cs.sonoma.edu/~dwells/movies/dbInterface.py?rType=moviesAtTheaters";
static NSString *citiesWithTheaters = @"http://cwolf.cs.sonoma.edu/~dwells/movies/dbInterface.py?rType=cityName";
...
{
    return _moviesReady && _theaterReady && _showtimeReady && _citiesReady && _moviesAtTheatersReady;
}
...
-(void) acceptWebData:(NSData *)webData forURL:(NSURL *)url
{
    ...
    if( [urlString isEqualToString: moviesURLString] ) {
        ...
        [self.downloadAssistant downloadContentsOfURL:[NSURL URLWithString:moviesAtTheaters]];
    }
    
    // make for cities
    else if( [urlString isEqualToString: citiesWithTheaters] ) {
        _citiesReady = YES;
        self.citiesDataSource = [[CitiesDataSource alloc] initWithJSONArray: [self jsonFromData:webData]];
        [self.downloadAssistant downloadContentsOfURL:[NSURL URLWithString:citiesWithTheaters]];
    }
    
    else if( [urlString isEqualToString: moviesAtTheaters] ) {
        _moviesAtTheatersReady = YES;
        self.moviesAtTheatersDataSource = [[MoviesAtTheatersDataSource alloc] initWithJSONArray:[self jsonFromData:webData]];
        ...
        self.dataSourceReady = _moviesReady && _theaterReady && _showtimeReady && _moviesAtTheatersReady;
    }
}
